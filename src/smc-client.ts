...
export class SMCClient {
  ...
  async queryAddressBook(keyword?: string): Promise<Terminal[]> {
    ...
    // 调用 SMC 3.0 接口
    const response = await this.request<SMCAddressBookResponse>(path, {
      method: 'GET'
    });
   ...
   * @returns Terminal 数组
   */
  private transformTerminals(response: SMCAddressBookResponse): Terminal[] {
